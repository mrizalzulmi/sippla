...
//---------- tambahanku ------------
    ...
    public function word()
    {
        if($this->session->userdata('logged_in')!="" && ($this->session->userdata('level')=="admin" or $this->session->userdata('level')=="pegawai")) {
            ...
            $this->load->view('spm/spm_doc',$data);
        } else {
            header('location:'.base_url().'');
        }
    }
    ...
    public function ambil_data_spmkasubid()
    {
        ...
        foreach($q->result_array() as $spm)
        {
            ...
            <tr>
                ...
                <td style="text-align:center"><?php echo anchor(site_url('upload/filepdf/'.$spm['FileUp']),'<i class="fa fa-file-pdf-o"></i>',array('title'=>'File Upload','class'=>'btn btn-warning btn-xs','target'=>'_blank')); ?></td>
