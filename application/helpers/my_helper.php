...
function gli($tabel, $field_kunci, $pad) {
	$CI 	=& get_instance();
	$nama	= $CI->multipledb->db3->query("SELECT max($field_kunci) AS last FROM $tabel")->row();
	$data	= (intval($nama->last)) + 1;
	$last	= str_pad($data, $pad, '0', STR_PAD_LEFT);
	return $last;
}

function gli2($tabel, $field_kunci, $pad, $year) {
	$CI 	=& get_instance();
	$nama	= $CI->multipledb->db3->query("SELECT ifnull(max($field_kunci),0) AS last FROM $tabel where year(tgl_surat)='$year'")->row();
	$data	= (intval($nama->last)) + 1;
	$last	= str_pad($data, $pad, '0', STR_PAD_LEFT);
	return $last;
}
...
function gval($tabel, $field_kunci, $diambil, $where) {
	$CI =& get_instance();	
	$nama	= $CI->multipledb->db3->query("SELECT $diambil FROM $tabel WHERE $field_kunci = '$where'")->row();
...
function getjenisnilai($id) {
	$CI =& get_instance();	
	$nama	= $CI->multipledb->db3->query("SELECT nama FROM tr_jenis_nilai WHERE id = '$id'")->row();
...
function getmapel($id) {
	$CI =& get_instance();	
	$nama	= $CI->multipledb->db3->query("SELECT nama FROM tr_mapel WHERE id = '$id'")->row();
	$data	= empty($nama) ? "-" : $nama->nama;
	return $data;
}
...
function getkelas($id) {
	$CI =& get_instance();	
	$nama	= $CI->multipledb->db3->query("SELECT nama FROM tr_kelas WHERE id = '$id'")->row();
	$data	= empty($nama) ? "-" : $nama->nama;
	return $data;
}
...
function getguru($id) {
	$CI =& get_instance();	
	$nama	= $CI->multipledb->db3->query("SELECT nama FROM tr_guru WHERE id = '$id'")->row();
	$data	= empty($nama) ? "-" : $nama->nama;
	return $data;
}
function getsiswa($id) {
	$CI =& get_instance();	
	$nama	= $CI->multipledb->db3->query("SELECT nama FROM tr_siswa WHERE id = '$id'")->row();
	$data	= empty($nama) ? "-" : $nama->nama;
	return $data;
}
